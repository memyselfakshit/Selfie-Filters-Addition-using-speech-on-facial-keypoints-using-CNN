...
def train_my_CNN_model(model, X_train, y_train):
    return model.fit(X_train, y_train, epochs=500, batch_size=500, verbose=1, validation_split=0.3)
...
def evaluate_my_CNN_model(model, X_test, y_test):
    return model.evaluate(X_test, y_test,  batch_size=500, verbose = 1)
